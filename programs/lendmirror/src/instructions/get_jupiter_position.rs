...
/// Shared Jupiter Position + Tick (+ branches) read used by get_jupiter_position
/// and refresh_wrapper. Branch accounts come in `remaining_accounts`.
pub(crate) fn compute_position_snapshot(
    vaults_program: &Pubkey,
    vault_id: u16,
    nft_id: u32,
    position_info: &AccountInfo,
    tick_info: &AccountInfo,
    vault_state_info: &AccountInfo,
    vault_config_info: &AccountInfo,
    tick_id_liquidation: Option<&AccountInfo>,
    remaining_accounts: &[AccountInfo],
) -> Result<PositionSnapshot> {
    let position = decode_position(&position_info.try_borrow_data()?)?;
    require!(
        position.vault_id == vault_id && position.nft_id == nft_id,
        LendMirrorError::PositionIdMismatch
    );

    let (expected_tick, _) = Pubkey::find_program_address(
        &[b"tick", &vault_id.to_le_bytes(), &tick_math::tick_pda_seed(position.tick)],
        vaults_program,
    );
    require!(tick_info.key() == expected_tick, LendMirrorError::TickPdaMismatch);

    let tick = decode_tick(&tick_info.try_borrow_data()?)?;
    require!(
        tick.vault_id == vault_id && tick.tick == normalize_tick(position.tick),
        LendMirrorError::TickPdaMismatch
    );
    let prices = decode_vault_state_prices(&vault_state_info.try_borrow_data()?)?;
    let tokens = decode_vault_tokens(&vault_config_info.try_borrow_data()?)?;
    ...
    if is_liquidated {
        let flushed = match tick_id_liquidation {
            Some(account) => {
        ...
        if record.is_fully_liquidated {
            ...
            live = walk_branches(
                ...
                |branch_id| {
                    let expected = branch_address(vaults_program, vault_id, branch_id);
                    ...
                    require!(
                        decoded.vault_id == vault_id && decoded.branch_id == branch_id,
                        LendMirrorError::InvalidJupiterAccount
                    );
                    Ok(Branch {
...
impl GetJupiterPosition<'_> {
    ...
    /// actually left. Writes `position_store` and `store.last_position`.
    pub fn apply(
