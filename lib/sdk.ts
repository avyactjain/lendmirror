...
export class CustomOAppSDK extends OmniSDK implements IOApp {
    ...
    protected readonly umiLendMirrorSdk: lendmirror.LendMirror
    ...
    constructor(
        ...
        this.umiLendMirrorSdk = new lendmirror.LendMirror(fromWeb3JsPublicKey(this.programId))
    }
    ...
    async getOwner(): Promise<OmniAddress> {
        ...
        const config = await mapError(
            () => {
                return lendmirror.accounts.fetchStore(this.umi, this.umiPublicKey)
            },
    ...
    async getPeer(eid: EndpointId): Promise<OmniAddress | undefined> {
        ...
        try {
            const peer = await lendmirror.getPeer(this.umi.rpc, eid, this.umiProgramId)
    ...
    async setPeer(eid: EndpointId, address: OmniAddress | null | undefined): Promise<OmniTransaction> {
        ...
        const umiTxs = [
            ...
            // this.umiLendMirrorSdk.setPeer(delegate, { peer: normalizedPeer, remoteEid: eid }),TODO: remove, since things are now handled by _createSetPeerAddressIx and _setPeerEnforcedOptionsIx
            await this._setPeerEnforcedOptionsIx(new Uint8Array([0, 3]), new Uint8Array([0, 3]), eid), // admin
            lendmirror.initOAppNonce({ admin: delegate, oapp }, eid, normalizedPeer), // delegate
        ]
        ...
        if (!isSendLibraryInitialized) {
            umiTxs.push(lendmirror.initSendLibrary({ admin: delegate, oapp }, eid))
        }

        if (!isReceiveLibraryInitialized) {
            umiTxs.push(lendmirror.initReceiveLibrary({ admin: delegate, oapp }, eid))
        }
    ...
    async getEnforcedOptions(eid: EndpointId, msgType: number): Promise<Bytes> {
        ...
        try {
            const options = await this.umiLendMirrorSdk.getEnforcedOptions(this.umi.rpc, eid)
    ...
    public async initConfig(eid: EndpointId): Promise<OmniTransaction | undefined> {
        ...
        if (!sendLibInitialized) {
            instructions.push(lendmirror.initSendLibrary({ admin: delegate, oapp: this.umiPublicKey }, eid))
        }

        if (!receiveLibInitialized) {
            instructions.push(lendmirror.initReceiveLibrary({ admin: delegate, oapp: this.umiPublicKey }, eid))
        }
    ...
    protected async _setPeerConfigIx(
        ...
        return this.umiLendMirrorSdk.setPeerConfig({ admin: await this._getAdmin() }, param)
    }
