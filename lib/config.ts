...
const createLendMirrorLogger = () => createModuleLogger('LendMirror')
const withLendMirrorLogger = createWithAsyncLogger(createLendMirrorLogger)
...
const onlyEdgesFromSolana = (
    ...
    const logger = createLendMirrorLogger()
...
export const initConfig: OAppConfigurator = createConfigureEdges(
    onlyEdgesFromSolana(
        withLendMirrorLogger(async ({ vector: { to } }, sdk) => {
            const logger = createLendMirrorLogger()
