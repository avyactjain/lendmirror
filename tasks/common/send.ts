...
async function sendFromSolana(fromEid: number, dstEid: number, message: string, computeUnitPriceScaleFactor: number) {
    ...
    const lendmirrorInstance: lendmirror.LendMirror = new lendmirror.LendMirror(publicKey(solanaDeployment.programId))

    const options = Options.newOptions().addExecutorLzReceiveOption(100000, 0).toBytes()

    const { nativeFee } = await lendmirrorInstance.quote(umi.rpc, umiWalletSigner.publicKey, {
        dstEid,
        message,
        options,
        payInLzToken: false,
    ...
    let txBuilder = transactionBuilder().add(
        await lendmirrorInstance.send(umi.rpc, umiWalletSigner.publicKey, {
            dstEid,
            message,
            options,
            nativeFee,
...
// LendMirror: Solana Devnet 40168 → Ethereum Sepolia 40161
task('lz:oapp:send', 'Sends a string message cross-chain', action)
    ...
    .addOptionalParam('contractName', 'Name of the OApp contract in deployments folder', 'LendMirror', types.string) // only if fromEid is EVM
