...
import { LendMirrorPDA, lendmirror } from '../../lib/client'
...
task('lz:oapp:solana:debug', 'Prints OApp Store and Peer information')
    ...
    .setAction(async (taskArgs: DebugTaskArgs) => {
        ...
        let storeInfo
        try {
            storeInfo = await lendmirror.accounts.fetchStore(umi, store)
        } catch (e) {
        ...
        const oappDeriver = new LendMirrorPDA(storeInfo.header.owner)
        ...
        const printStore = async () => {
            ...
            DebugLogger.keyValue('Vaults Program', storeInfo.vaultsProgram)
            const snap = unwrapOption(storeInfo.lastPosition)
            if (snap) {
                DebugLogger.keyValue('position', String(snap.position))
                DebugLogger.keyValue('vaultId', String(snap.vaultId))
                DebugLogger.keyValue('nftId', String(snap.nftId))
                DebugLogger.keyValue('colRaw', String(snap.colRaw))
                DebugLogger.keyValue('debtRaw', String(snap.debtRaw))
                DebugLogger.keyValue('netDebt', String(snap.netDebt))
                DebugLogger.keyValue('tick', String(snap.tick))
                DebugLogger.keyValue('storedColRaw', String(snap.storedColRaw))
                DebugLogger.keyValue('storedDebtRaw', String(snap.storedDebtRaw))
                DebugLogger.keyValue('storedTick', String(snap.storedTick))
                DebugLogger.keyValue('isLiquidated', String(snap.isLiquidated))
                DebugLogger.keyValue('isFullyLiquidated', String(snap.isFullyLiquidated))
                DebugLogger.keyValue('branchId', String(snap.branchId))
                DebugLogger.keyValue('snapshotTime', String(snap.snapshotTime))
            } else {
                DebugLogger.keyValue('lastPosition', 'none')
            }
        ...
        const printPeerConfigs = async () => {
            ...
            const peerConfigInfos = await lendmirror.accounts.fetchAllPeerConfig(umi, peerConfigs)
